...
do
    result_allreduce1=$(mpirun --map-by core -np $idx_process --mca coll_tuned_use_dynamic_rules true --mca coll_tuned_reduce_algorithm 4 $OSU_REDUCE -m $dimension_size -x $N_replica -i $N_replica | tail -n 1 | awk '{print $2}') # osu_allreduce with current processors, fixed message dimension_size and fixed number of N_replica
