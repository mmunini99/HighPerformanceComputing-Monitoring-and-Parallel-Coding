...
int main(int argc, char *argv[])
{
    ...
    int base_strips = img_height / total_processes;
    int extra_strips = img_height % total_processes;
    ...
    for (int strip = 0; strip < local_strips; ++strip) {
        ...
        double coord_y = min_y + global_strip * (max_y - min_y) / img_height;
    ...
    if (process_id == 0) {
        ...
        for (int proc = 0, offset = 0; proc < total_processes; ++proc) {
            ...
            receive_sizes[proc] = strips * img_width;
            data_offsets[proc] = offset;
            offset += strips * img_width;
        }
    }
    
    MPI_Gatherv(local_buffer, local_strips * img_width, MPI_UNSIGNED_CHAR,
    ...
    if (process_id == 0) {
        ...
        FILE *output_file = fopen("fractal_row.pgm", "wb");
        if (output_file) {
            fprintf(output_file, "P5\n%d %d\n255\n", img_width, img_height);
            fwrite(final_image, 1, img_width * img_height, output_file);
            fclose(output_file);
        }
